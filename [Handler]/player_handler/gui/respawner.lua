...
local respawnerUI = {
    ...
    vignetteTexture = imports.beautify.native.createTexture("files/images/hud/vignette.png", "dxt5", true, "clamp")
}
        ...
        if elapsedDuration >= (FRAMEWORK_CONFIGS["UI"]["Loading"].fadeInDuration + FRAMEWORK_CONFIGS["UI"]["Loading"].fadeDelayDuration) then
            ...
            for i = 1, 360 do
                ...
                if generatedSpawnPoint.x and generatedSpawnPoint.y and generatedSpawnPoint.z and generatedSpawnPoint.z ~= 0 then
                    triggerServerEvent("Player:onSpawn", localPlayer, generatedSpawnPoint)
                    hideRespawnScreen()
                    return false
                end
            end
