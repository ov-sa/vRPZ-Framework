...
CGame.execOnModuleLoad(function()
    ...
    local nametagUI = {
        ...
        clipRange = {4, 6}, weatherBlend = 0.95,
    ...
    nametagUI.createUI = function(player, isReload)
        ...
        nametagUI.buffer[player].reputation = imports.assetify.createShader(nametagUI.buffer[player].rt, "player-nametag", "Assetify_TextureGrayscaler", nil,
            {},
            {},
            {},
    ...
    imports.assetify.network:fetch("Assetify:onEntityDataChange", true):on(function(element, data, _, value)
        if value then
            if (data == "Character:Data:level") or (data == "Character:Data:reputation") then
                nametagUI.updateUI(element)
            end
        end
    end)
end)