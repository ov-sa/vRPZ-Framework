...
shaderRW.buffer[(identity.name)] = {
    ...
    exec = function()
        ...
        float4 SampleEmissive(float2 uv) {
            ...
            float4 result = tex2D(vSource0Sampler, float4(uv, 0, 0))*viewBrightness;
            for(float i = 0; i < viewPI; i += viewPI/viewIterations) {
                for(float j = 1/viewQuality; j <= 1; j += 1/viewQuality) {
                    result += tex2D(vSource0Sampler, float4(uv + (float2(cos(i), sin(i))*viewRadius*j), 0, 0))*viewBrightness;
                }
            }
