...
local testImage = dxCreateTexture("models/corona2.png")
...
function fetch()
    return testShader, testImage
end

addEventHandler("onClientPreRender", root, function()
    local rts = assetify.renderer.getVirtualRTs()
    if rts and rts.diffuse and isElement(rts.diffuse) then
        dxDrawImage(0, 0, 1366*0.45, 768*0.45, rts.diffuse)
    end
